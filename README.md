# Peasant

> **"Finance for the rest of us"** 💰  
> Democratizing wealth-building tools that the upper class has always had access to

---

## 🎯 What Is Peasant?

Peasant is a modular financial super app designed for working-class people. It teaches you the financial tricks that wealthy people already know and gives you the tools to escape the paycheck-to-paycheck trap.

### The Mission

**Stop being poor because you don't know what rich people know.**

Peasant breaks down the financial gatekeeping and gives you:
- The knowledge wealthy people learn from their parents
- The tools financial advisors charge thousands for
- The tricks that keep you poor (and how to stop them)
- A community of people who get it

---

## 🧩 The Plugin Ecosystem

Peasant is built on a modular plugin system. Each plugin solves one specific financial problem.

**Core Plugins:**
1. **Budget Repair & Balance** - Track spending, kill subscriptions, optimize pay periods
2. **Credit Repair Toolkit** - From 580 to 750 credit score
3. **Debt Destroyer** - Get out of debt without bankruptcy
4. **Car Buying Mastery** - Never get ripped off at a dealership
5. **Health Insurance Optimizer** - HSA vs PPO, stop overpaying
6. **Investment Starter** - From peasant to investor
7. **Side Hustle Tracker** - Turn skills into cash
8. **Home Buying Calculator** - Rent vs buy, the real math
9. **Fast Food Killer** - Scan receipts, get recipes, save $6K+/year

**Platform Plugins:**
10. **Peasant Pay** - Payments and transfers
11. **Peasant Banking** - Your bank, but better
12. **Peasant University** - Financial education
13. **Peasant Community** - Connect with other peasants

See [PLUGINS.md](./PLUGINS.md) for complete details.

---

## 🚀 Status

**Current:** Planning & Architecture  
**Next:** Build Budget plugin (foundation)  
**Future:** Full plugin marketplace

This is a work in progress. The vision is big, but we're starting with the foundation.

---

## 🎓 Learning to Build Peasant

Want to learn how to build apps like this?

Check out [sudo-make-me-a-developer](https://github.com/ProvenGuilty/sudo-make-me-a-developer) - a free, open-source course that teaches you to build the Budget plugin from scratch using AI-assisted development.

---

## 📚 Documentation

- [Vision](./VISION.md) - Long-term roadmap and strategy
- [Plugins](./PLUGINS.md) - Complete plugin ecosystem
- [Architecture](./ARCHITECTURE.md) - Technical design (coming soon)
- [Contributing](./CONTRIBUTING.md) - How to contribute (coming soon)

---

## 💡 Philosophy

**Anti-Gatekeeping**
- No financial jargon
- Plain English explanations
- Humor and humility
- "We're all in this together"

**Education-First**
- Not just tools, but teaching
- Learn the tricks wealthy people know
- Understand why, not just how
- Empower, don't just serve

**Community-Driven**
- Built by peasants, for peasants
- Open to contributions
- Success stories inspire others
- Rising tide lifts all boats

**AI-Powered**
- Smart categorization
- Personalized advice
- Negotiation scripts
- Predictive insights

---

## 🤝 Contributing

This project is in early stages. Contributions welcome once we have the foundation built.

For now, the best way to contribute is to:
1. Learn to build it: [sudo-make-me-a-developer](https://github.com/ProvenGuilty/sudo-make-me-a-developer)
2. Build your own version
3. Share what you learned
4. Help others learn

---

## 📜 License

TBD - Will be determined as project matures

---

## 🌟 The Vision

Start with a budget app. Grow into a super app. Become the financial platform for working-class people.

**Year 1:** Budget, Credit, Debt plugins  
**Year 2:** Car, Healthcare, Investing plugins  
**Year 3:** Payments, Banking platform  
**Year 4:** Full ecosystem  
**Year 5:** Financial super app for the people

See [VISION.md](./VISION.md) for the complete roadmap.

---

**PEASANT: Finance for the rest of us** 💰

*Because we're all just peasants with smartphones, trying to make it in a world designed for the wealthy.*

---

*Repository created: November 2025*
